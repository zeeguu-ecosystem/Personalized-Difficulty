...
from sklearn.linear_model import LinearRegression, Ridge, Lasso, ElasticNet, ElasticNetCV, LogisticRegression, \
    Perceptron, SGDRegressor, BayesianRidge
import numpy as np
    ...
    for name, clf in zip(names, classifiers):
        ...
        for i in range(1):
            classifier_pipeline = make_pipeline(StandardScaler(),
                                                SelectKBest(score_func=f_regression, k=fs),clf)

            predicted = cross_val_predict(classifier_pipeline, X, y, cv=KFold(n_splits=10,shuffle=True,random_state=i))

            r2_scores.append(mean_squared_error(y, predicted))

        print(name,fs, mean(r2_scores), np.var(r2_scores))
