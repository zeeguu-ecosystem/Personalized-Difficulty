...
for language in languages:
    if language.name in ['Dutch', 'Spanish','Danish','Romanian']:
        print(language.name, 'skipped')
        continue
...
for ua in articles:
    ...
    if ua.language.name not in list(lang_total.keys()):
        continue
    ...
    N_words[ua.language.name].append(ua.fk_difficulty)
...
fig['layout']['yaxis1'].update(range = [0, 1], title = 'Word interaction score')
fig['layout']['xaxis1'].update(title = 'Flesch-Kincaid readability ease')
...
fig["layout"].update(
        title= "Word interaction score vs Flesch-Kincaid",
