import zeeguu
from zeeguu.model import UserArticle, User, Language, Article, Bookmark
...
user_art = defaultdict(int)
for bm in Bookmark.query.all():
    user_art[str(bm.user_id) + str(bm.text_id)] += 1

print(sum(user_art.values()),len(user_art.values()))
...
for ua in userarticles:
    ...
    difficulty[ua.language.name].append(ua.fk_difficulty)
...
fig['layout']['xaxis1'].update(title = 'Flesch-Kincaid')
