        ...
        for act in activity:
            ...
            for art in articles:
                ...
                if art.url.domain.domain_name + art.url.path == url:
                    ...
                    if user.name + art.language.name not in estimator.keys():
                        estimator[user.name + art.language.name] = CognacyDifficultyEstimator.cognacyRatio(art.language, user)

                    diff_est = estimator[user.name + art.language.name].estimate_difficulty(art.content)
                    article_url.append(art)
                    scores[eval].append(diff_est['normalized'])
...
fig["layout"].update(
        title= "Cognate ratio evaluations",
        hovermode= 'closest',
        showlegend=False,
